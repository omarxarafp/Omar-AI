...
async function searchAPKPure(query, num = 10) {
    try {
        const [searchResponse, an1Response, apkmbResponse] = await Promise.all([
        ...
        const combined = [...normalResults, ...an1Results, ...apkmbResults];
        console.log(`[Search] Found ${normalResults.length} from APKPure + ${an1Results.length} from AN1 + ${apkmbResults.length} from GetModsAPK`);
        return combined;
    } catch (error) {
...
◄ https://www.instagram.com/omarxarafp${POWERED_BY}` 
    ...
    // Check if this is a mod download
    ...
    // Handle mod download (AN1, GetModsAPK)
    if (isModDownload) {
